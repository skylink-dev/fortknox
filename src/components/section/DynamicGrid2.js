...
export default function DynamicGrid2({ data }) {
  const leftContent = data.cards[0]?.subcards?.[0];
  let gridContent = data.cards[1]?.subcards;
  return (
          ...
          >
            <div className="bg-gray-100 text-center rounded-lg shadow-sm hover:shadow-md transition duration-300 flex flex-col items-center relative overflow-hidden max-h-[260px]">
              <Image
                src={item.image}
                alt={item.title}
                className="mx-auto object-cover min-h-[280px]"
                width={500}
                height={400}
                layout="intrinsic"
              />
            </div>
            <h4 className="font-semibold mb-1 text-center mt-2 text-[16px] pb-2">
              {item.title}
            </h4>
          </a>
        ))}
